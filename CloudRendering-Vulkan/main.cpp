...
void UpdateTime()
{
	...
	if (pushConstants.time - g_previousTime >= 1.0)
	{
...
void RecordFrameCommands(uint32_t imageIndex)
{
	...
	// Push constants
	vkCmdPushConstants(commandBuffer, g_pathTracerPipelineLayout->GetPipelineLayout(), VK_SHADER_STAGE_COMPUTE_BIT, 0, sizeof(PushConstants), &pushConstants);
...
{
	shadowVolumeProperties.SetLightDirection(g_lightDirection);
...
void CreateSwapchain()
{
	...
	g_pathTracerImage = new VulkanImage(g_device, VK_FORMAT_R32G32B32A32_SFLOAT, VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_STORAGE_BIT, static_cast<uint32_t>(cameraProperties.width), static_cast<uint32_t>(cameraProperties.height));
...
void AllocateShaderResources()
{
	...
	// Buffers & Images
	cameraPropertiesBuffer = new VulkanBuffer(g_device, &cameraProperties, sizeof(CameraProperties), VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT);
	cloudPropertiesBuffer = new VulkanBuffer(g_device, &cloudProperties, sizeof(CloudProperties), VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT);
	parametersBuffer = new VulkanBuffer(g_device, &parameters, sizeof(Parameters), VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT);
	...
	shadowVolumePropertiesBuffer = new VulkanBuffer(g_device, &shadowVolumeProperties, sizeof(ShadowVolumeProperties), VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT);
	g_shadowVolumeImage = new VulkanImage(g_device, VK_FORMAT_R32_SFLOAT, VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_STORAGE_BIT, shadowVolumeProperties.voxelAxisCount, shadowVolumeProperties.voxelAxisCount, shadowVolumeProperties.voxelAxisCount);
...
bool InitializeWindow()
{
	...
	g_window = glfwCreateWindow(cameraProperties.width, cameraProperties.height, "Cloud Renderer", nullptr, nullptr);
...
int main()
{
	...
	glm::vec3 cloudSize{
		g_cloudData->GetVoxelSize().x * g_cloudData->GetVoxelCount().x * cloudProperties.baseScaling,
		g_cloudData->GetVoxelSize().y * g_cloudData->GetVoxelCount().y * cloudProperties.baseScaling,
		g_cloudData->GetVoxelSize().z * g_cloudData->GetVoxelCount().z * cloudProperties.baseScaling };

	cloudProperties.maxExtinction = g_cloudData->GetMajorant();
	cloudProperties.voxelCount = glm::uvec4(g_cloudData->GetVoxelCount(), 0);
